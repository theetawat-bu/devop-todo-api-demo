...
`USER node` ตัดขั้นแรกของบันไดนี้ทิ้ง
...
docker run --rm --read-only --tmpfs /tmp \
  -e DATABASE_URL=... -p 3000:3000 devops-todo-api:local
```

**ทำไมต้องมี `--tmpfs /tmp`:** Node และ library หลายตัวเขียนไฟล์ชั่วคราวลง `/tmp` ถ้าเขียนไม่ได้จะพัง
...
```bash
docker run -d --name oom --memory=64m -e DATABASE_URL=... devops-todo-api:local
# ยิงโหลดหรือรอให้ Node ใช้ heap เกิน
...
```bash
docker run -d --name slow --cpus=0.2 -e DATABASE_URL=... -p 3001:3000 devops-todo-api:local
...
| **npm package** | `express`, `semver` | `npm audit fix` หรืออัปเดต dependency |
...
```dockerfile
FROM node:22-alpine@sha256:abc123… AS builder
```

**ทำไม tag ไม่ปลอดภัย:** `node:22-alpine` วันนี้กับพรุ่งนี้อาจเป็นคนละ image — เจ้าของ repo push ทับ tag เดิมได้ตลอด
...
```bash
docker run -d --name g -e DATABASE_URL=... devops-todo-api:local
time docker stop g       # ~0.5 วิ

# คอมเมนต์ process.on('SIGTERM', ...) ใน src/index.ts แล้ว build ใหม่
...
```
docker stop → SIGTERM → แอปดักได้ → server.close() → รอ request ที่ค้างจนจบ
           → prisma.$disconnect() → process.exit(0)   ← จบใน 0.5 วิ
...
โปรเจกต์นี้ log เป็น JSON อยู่แล้วใน `src/app.ts`:
